...
<body class="bg-light">
        ...
        <table class="table table-sm table-bordered fs-15 mt-3">
            ...
            @forelse ($order as $a)
              <tr>
                ...
                <td>{{ $a->jenis_cucian}}</td>
                <td>{{ $a->qty}}</td>
                <td>
                  <button class="btn btn-sm btn-danger fs-15" onclick="hapus({{ $a->id }})"><i class="bi-trash"></i></button>
    ...
    <script>
      function hapus(id){
        var a = confirm('Apakah yakin ingin menghapus data ini ?');
        if(a){
          window.location.href = '/kasir/order/hapus/'+id;
        }
      }
    </script>
</body>
